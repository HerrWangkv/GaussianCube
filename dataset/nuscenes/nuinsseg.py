# Adapted from https://github.com/Serenos/nuInsSeg/blob/main/nuinsseg-devkit/nuinsseg.py
...
class NuInsSeg(NuScenes):
    '''
    '''
    def __init__(self, version: str = 'v1.0-mini', data_root: str = '/data/sets/nuscenes', ins_seg_root: str = '/data/sets/nuscenes', verbose: bool = True, map_resolution: float = 0.1):
        super().__init__(version, data_root, verbose, map_resolution)
        # If available, also load the instance mask table created by export_2d_annotations_as_json().
        json_path = os.path.join(ins_seg_root, version, 'nuinsseg.json')
        if os.path.exists(json_path):
            new_table = 'nuinsseg'
            self.table_names.append(new_table)
            with open(os.path.join(json_path)) as f:
                self.nuinsseg = json.load(f)
            ...
            raise FileNotFoundError('nuInsSeg dataset not found at {}'.format(json_path))

    def get_nuinsseg(self, ann_token, sd_token):
        index = self._token2ind['nuinsseg'][ann_token, sd_token]
        return self.nuinsseg[index]

    def is_visible(self, sample, ann_token, min_visibility=4):
        for cam in CAM_SENSOR:
            sd_token = sample['data'][cam]
            if (ann_token, sd_token) in list(self._token2ind['nuinsseg'].keys()):
                instance_rec = self.get_nuinsseg(ann_token, sd_token)
                if len(instance_rec['instance_mask']) != 0 and int(instance_rec['visibility_token']) >= min_visibility:
                    return True
        return False

    def get_masks(self, sample, ann_token):
        masks = dict()
        for cam in CAM_SENSOR:
            sd_token = sample['data'][cam]
            if (ann_token, sd_token) in list(self._token2ind['nuinsseg'].keys()):
...
if __name__ == '__main__':
    data_root = os.path.join(os.environ['HOME'], 'datasets/nuscenes')
    nuseg = NuInsSeg(version='v1.0-mini', data_root=data_root, verbose=True)
