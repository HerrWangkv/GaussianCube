...
def main():
    ...
    assert "controlnet" in model_and_diffusion_config, "Controlnet config is missing in the model config"
    ...
    controlnet.load_state_dict(torch.load(args.controlnet_ckpt, map_location="cpu"))
    print("Loaded controlnet ckpt: ", args.controlnet_ckpt)
    ...
    model.eval()
    print("num of unet params: {} M".format(sum(p.numel() for p in model.parameters() if p.requires_grad)/1e6))
    controlnet.to(dist_util.dev())
    controlnet.eval()
    print("num of control net params: {} M".format(sum(p.numel() for p in controlnet.parameters() if p.requires_grad)/1e6))
    overall_model.to(dist_util.dev())
    overall_model.eval()
    print("num of overall params: {} M".format(sum(p.numel() for p in overall_model.parameters() if p.requires_grad)/1e6))
    ...
    for _ in tqdm(range(num_batch_per_rank)):
        ...
        pretrained_dpm_solver = DPM_Solver(pretrained_model_fn, noise_schedule, algorithm_type='dpmsolver++')
        ...
        finetuned_dpm_solver = DPM_Solver(finetuned_model_fn, noise_schedule, algorithm_type='dpmsolver++')

        with torch.no_grad():
            noise = torch.randn(sample_shape, device=dist_util.dev()) * args.temperature

            pretrained_samples = pretrained_dpm_solver.sample(
                x=noise,
            ...
            finetuned_samples = finetuned_dpm_solver.sample(
                x=noise,
                steps=args.rescale_timesteps,
                t_start=1.0,
                t_end=1/1000,
            ...
            for i, cam_info in enumerate(model_kwargs["cams"]):
                ...
                res = render(cam, pretrained_samples_denorm[0], std_volume, bg_color, args.active_sh_degree)
                
                s_path = os.path.join(logger.get_dir(), 'pretrained_render_images')
                ...
                pretrained_frames.append(rgb_map)
        
            # pretrained_samples = pretrained_samples.permute(0, 2, 3, 4, 1).cpu()
            # torch.save(pretrained_samples[0], os.path.join(logger.get_dir(), f"pretrained_rank_{dist.get_rank():02}_{img_id:04d}" + ".pt"))
            if args.render_video:
                s_path = os.path.join(logger.get_dir(), 'pretrained_videos')
                os.makedirs(s_path,exist_ok=True)
                imageio.mimwrite(os.path.join(s_path, "rank_{:02}_render_{:06}.mp4".format(dist.get_rank(), img_id)), pretrained_frames, fps=30)

            finetuned_frames = []
            for i, cam_info in enumerate(model_kwargs["cams"]):
                cam = build_single_viewpoint_cam(cam_info, 0)
                res = render(cam, finetuned_samples_denorm[0], std_volume, bg_color, args.active_sh_degree)
                
                s_path = os.path.join(logger.get_dir(), 'finetuned_render_images')
                os.makedirs(s_path,exist_ok=True)
                output_image = res["render"].clamp(0.0, 1.0)
                ...
                finetuned_frames.append(rgb_map)
        
            # finetuned_samples = finetuned_samples.permute(0, 2, 3, 4, 1).cpu()
            # torch.save(finetuned_samples[0], os.path.join(logger.get_dir(), f"finetuned_rank_{dist.get_rank():02}_{img_id:04d}" + ".pt"))
            if args.render_video:
                s_path = os.path.join(logger.get_dir(), 'finetuned_videos')
                os.makedirs(s_path,exist_ok=True)
                imageio.mimwrite(os.path.join(s_path, "rank_{:02}_render_{:06}.mp4".format(dist.get_rank(), img_id)), finetuned_frames, fps=30)
