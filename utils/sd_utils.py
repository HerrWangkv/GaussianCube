...
class StableDiffusion(nn.Module):
    ...
    @torch.no_grad()
    def refine(self, prompts, negative_prompts, pred_rgb, timesteps=199):
        ...
        neg_embeds = self.get_text_embeds(negative_prompts) # [1, 77, 768]
        ...
        t = torch.tensor([timesteps] * latents.shape[0], dtype=torch.long, device=self.device)
        latents_noisy = self.scheduler.add_noise(latents, noise, t)

        self.scheduler.set_timesteps(100)
        # produce_latents
        for i, t in enumerate(range(timesteps, 0, -10)):
            latent_model_input = torch.cat([latents_noisy] * 2)
            noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeds).sample

            # perform guidance
            noise_pred_uncond, noise_pred_pos = noise_pred.chunk(2)
            noise_pred = noise_pred_uncond + 50 * (noise_pred_pos - noise_pred_uncond)
...
if __name__ == '__main__':
    ...
    rgb = Image.open("/home/kwang/test/GaussianCube/experiments/objaverse_test/render_images/rank_00_render_000000_cam_25.png")
